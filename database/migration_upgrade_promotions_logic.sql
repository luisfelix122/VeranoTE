-- Migración para mejorar la lógica de promociones con filtros de categoría y recurso
-- Actualiza calcular_descuento_simulado para usar IDs técnicos
...
BEGIN
    ...
    FOR v_promo IN SELECT * FROM PROMOCIONES WHERE activo = TRUE LOOP
        ...
        -- Lógica de aplicación básica (Automática vs Cupón)
        ...
        IF v_aplicar THEN
            ...
            -- Filtros específicos de la promoción
            v_filtro_cat := v_promo.condicion->>'categoria_id';
            v_filtro_recurso := v_promo.condicion->>'recurso_id';
            ...
            IF v_promo.tipo = 'regla_tiempo' THEN
                ...
                FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
                     ...
                     SELECT precio_por_hora, categoria_id 
                     INTO v_precio_hora, v_categoria_id 
                     FROM RECURSOS WHERE id = v_recurso_id_item;
                     
                     IF FOUND THEN 
                        -- Aplicar filtros
                        IF (v_filtro_recurso IS NULL OR v_filtro_recurso = '' OR v_filtro_recurso::INTEGER = v_recurso_id_item) AND
                           (v_filtro_cat IS NULL OR v_filtro_cat = '' OR v_filtro_cat::INTEGER = v_categoria_id) THEN
                ...
                ELSE
                    -- UPSELLING for Time Rules (solo si es global o coincide con el item del carrito)
                    IF v_promo.es_automatico IS TRUE AND (v_filtro_recurso IS NULL OR v_filtro_recurso = '') AND (v_filtro_cat IS NULL OR v_filtro_cat = '') THEN
    ...
    BEGIN
        ...
        RETURN jsonb_build_object(
            ...
            'subtotal_base', v_base_imponible,
            'igv', v_igv,
            'subtotal', v_total_servicio,
